...
## ФІКС: помилка при зміні статусу запису — 2026-06-14
- Симптом: при зміні статусу (напр. на «Виконано») інколи виринала помилка.
- Причина: холодний старт Render (free) — перший запит будить інстанс (~30с, 404
  `x-render-routing: no-server`). GET-запити ретраїлись, а **PATCH `/status` — ні** → падав.
  Бекенд+CORS перевірено: PATCH→200, заголовки коректні (тобто баг суто в клієнті на cold start).
- Фікс: `frontend/src/lib/api.js` — ретраї тепер для всіх ІДЕМПОТЕНТНИХ методів
  (GET/PATCH/PUT/DELETE), POST лишається без ретраїв (щоб не дублювати записи).
