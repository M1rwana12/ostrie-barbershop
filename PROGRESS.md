...
## POSTGRES — ПІДКЛЮЧЕНО ✅ (2026-06-14)
- [x] Перевірено через Render API: сервіс `srv-d8mpanrtqb8s73cd0ct0` має env `DATABASE_URL`
  → внутрішній рядок Postgres `ostrie-db` (`dpg-d8mtg9btqb8s73chrv50-a`, oregon, free,
  expires ~2026-07-13), `JWT_SECRET` згенеровано. БД status=available. Дані ПЕРСИСТЕНТНІ
  (super-admin created_at стабільний між деплоями). SUPER_ADMIN_* в env немає → супер-адмін
  сидиться з зашитих config-дефолтів (senja3209@icloud.com + bcrypt-хеш). Тепер зміна пароля
  в панелі зберігається — варто змінити дефолтний пароль.

## РЕДИЗАЙН АДМІНКИ 2.0 (sidebar + dashboard + UX) — 2026-06-14
- [x] Каркас: `Admin.jsx` переписано на **sidebar + topbar + content**. Сайдбар з іконками
  (role-gated), topbar з назвою розділу + перемикач мови + меню користувача (аватар-ініціал,
  dropdown: Безпека/Вийти). Мобільний drawer (бургер+overlay). Екран входу — окремий `.admin-auth`.
- [x] **Dashboard «Огляд»** (`AdminDashboard.jsx`): KPI-плитки (виручка 30д для super, усього/нові/
  виконано), SVG area-графік виручки за днями (без ліб), записи на сьогодні, стрічка останніх подій
  аудиту. Плитки клікабельні → перехід у розділи.
- [x] **UX-фундамент**: тост-система (`lib/toast.jsx`, ToastProvider+useToast) на дії
  (статус/користувачі/пароль/2FA); скелетон-лоадери (`Skeleton.jsx`) у таблицях/картках;
  набір line-іконок (`icons.jsx`); токени кольорів `--ok/--danger/--warn`; формат чисел/грошей
  з локаллю (`lib/format.js`, `toLocaleString`, відносний час).
- [x] **Записи**: пошук (імʼя/телефон/послуга) + чипси-фільтр за статусом + скелетон + тости.
- [x] Аналітика/Users/Audit/Security: формат грошей, скелетони, тости. Дублюючі H1 прибрано
  (topbar показує розділ). Білд OK (59 модулів).

## ФІКС: «Input should be a valid dictionary…» при зміні статусу — 2026-06-14
- Симптом: зміна статусу запису → 422 `Input should be a valid dictionary or object`.
- СПРАВЖНЯ ПРИЧИНА (баг у `api.js`): у `fetch` ключ `headers` стояв ПЕРЕД `...options`,
  тож `options.headers` з `auth()` (Authorization, без Content-Type) ЗАТИРАВ
  `Content-Type: application/json`. Браузер слав тіло як `text/plain` → FastAPI не парсив
  його як обʼєкт. Ламались УСІ авторизовані запити з тілом (статус, create user, зміна
  пароля, 2FA). POST-запис працював лише тому, що не мав headers в options.
- Фікс: у `request()` тепер `headers` ставиться ПІСЛЯ `...rest` (деструктуризація options),
  Content-Type гарантовано зберігається. Підтверджено на проді: text/plain→422, json→200.
- Бонус того ж дня: ретраї холодного старту Render тепер і для ІДЕМПОТЕНТНИХ методів
  (GET/PATCH/PUT/DELETE), POST без ретраїв (щоб не дублювати записи).

## ЛЕНДИНГ — TIER B (унікальність/вау) — 2026-06-14
Після аудиту сайту користувач обрав Tier B. Реалізовано 3 сигнатурні фішки:
- [x] **Blade-wipe переходи**: заголовки секцій (`.sec-head .display`, `.booking-aside .display`)
  «врізаються» через `clip-path` + брас-лезо, що сяє й пролітає по заголовку (CSS-only,
  `prefers-reduced-motion: no-preference`, тригер на `.in` від існуючого reveal-observer).
- [x] **«Звук закладу»** — Spotify-плейлист (iframe, theme=0) у секції About (нова `.sound`),
  i18n `about.sound*`. ⚠️ ЗАРАЗ ПЛЕЙЛИСТ-ПЛЕЙСХОЛДЕР (Rock Classics `37i9...26U`) — замінити
  на власний плейлист закладу.
- [x] **«Завантаженість сьогодні»** (`TodayLoad.jsx`) — жива шкала по годинах: агрегує
  `GET /availability` по всіх майстрах на сьогодні, показує free/busy/full + «вільно ще N
  слотів». Вбудовано в aside секції Запис. i18n `booking.load*`.
- Білд OK. Наступне з Tier B (не зроблено): візуальний конструктор образу з фото.
